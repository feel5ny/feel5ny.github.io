...
date: '2020-04-05'
description: >-
  TL;DR ⭐️⭐️⭐️ 1. 리팩토링하기 전에 제대로 된 테스트부터 마련한다. 테스트는 반드시 자가진단하도록
  만든다. 2. 프로그램이 새로운 기능을 추가하기에 편한 구조가 아니라면, 먼저 기능을 추가하기 쉬운
  형태로 리팩터링하고 나서 원하는 기능을 추가한다. 3. 리팩...
author: Joy Kim
categories:
  - 00_Clean_Code
  - Refactoring
...
customList: |-
  [
    ...
    },
    {
      "title": {
        "name": "골격 다시 잡기",
        "sub": "함수 추출하기, 변수명 변경, 변수 인라인으로 변경, 반복문 쪼개기",
        "id": "architect",
        "style": {
          "type": "number",
          "index": "2"
    ...
    },
    {
      "title": {
        "name": "사람이 이해하도록 작성하기",
        "sub": "임시변수를 질의함수로 바꾸기, 변수 인라인하기, 함수명 바꾸기, 반복문 쪼개기, 문장 슬라이드하기",
        "id": "easy",
        "style": {
          "type": "number",
          "index": "4"
    ...
    {
      "title": {
        "name": "작업 시작 전보다 건강한 코드로 유지하기",
        "sub": "단계 쪼개기, 함수 옮기기, 반복문을 파이프라인으로 바꾸기",
        "id": "healthy",
        "style": {
          "type": "number",
          "index": "5"
    ...
    },
    {
      "title": {
        "name": "수정하기 쉽도록 작성하기",
        "sub": "조건부 로직을 다형성으로 바꾸기, 타입코드를 서브클래스로 바꾸기, 생성자 팩터리 함수로 바꾸기",
        "id": "Polymorphism",
        "style": {
          "type": "number",
          "index": "6"
