---
title: 코드스피츠85 2회-(2) 동시성 모델을 직접 구현하며 이해하기.
date: '2019-11-11'
description: "코드스피츠 85에서는 none blocking에 대한 이야기와 자바스크립트를 짜는 근본적인 방법에 대한 고찰을 이야기해본다. --- \U0001F315\U0001F311\U0001F311 TL;DR setTimer에서부터 promise까지 동시성 모델을 기반으로 구현하며, 루프 제어권의 통제에 대하여 알아본다...."
author: Joy Kim
tags:
  - javaScript
  - codeSpitz
...
<details>
  ...
  <ul>
    <li>
      ...
      <ul>
        <li>브라우저가 실행되 이후에 지난 시간</li>
        <li>date.now()보다 좋은 점은 나노초까지 볼 수 있다.</li>
      </ul>
    </li>
    <li>
      <code>Set</code>
      <ul>
        <li>배열에 담을 수 있는건 값만 담을 수 있다.</li>
        <li>같은 객체가 중복으로 들어가지 않는다.</li>
        <li>객체를 담는 리스트</li>
      </ul>
    </li>
    <li>
...
- 브라우저에게 수행하기를 원하는 애니메이션을 알리고, 다음 리페인트가 진행되기 전에 해당
  애니메이션을 업데이트하는 함수를 호출하게 한다.
...
1. `requestAnimationFrame`으로 내부함수 `f`가 실행
2. load기준으로 반복 카운트가 chunk된다.
3. while문이 한 셋트가(load 카운트가 종료) 끝나면, `requestAnimationFrame`으로 `f`함수를 다시
   실행시킨다.
...
- [제너레이터 글참고](https://feel5ny.github.io/2019/02/03/JS_19/)
...
- 제너레이터는 **유사 iterable**이다.
  ...
  - generator는 `for...of`를 사용하지 못한다. for...of는 iterable이 와야하기 때문에
- `yield`가 일어날 때마다 `next`로 다음 턴을 줄 수 있다.
...
- **yield를 호출하면 suspend가 일어난다.**
  ...
  - 다음번 next 호출시 내부적으로 다시 재개되어서 루프돈다.
    - next 호출할때마다 `suspend`가 일어난 곳에서 `resume`이 일어난다.
    - 멈추는 것: `suspend`
    - 다시 재개: `resume`
...
**제어 시스템의 반제어권을 외부에 줌으로써** 내부에서 제어와 관련된 로직을 분리시킬수 있게 된다는게
제너레이터의 장점
...
- Promise에 바로 `then`을 사용하는 것은 반제어권이 이점을 활용하지 않고 **콜백처럼 쓰는 형태**
...
- 제어권을 완전 양도했었지만 위 코드는 capsulizing해서 Promise안의 작업이 끝나면 then을 호출할 수
  있게끔 반제어권을 주었음.
